...
const doorBaseColors: Record<DoorType, string> = {
  white: "#e5e5e5",
  black: "#1f2937",
  red: "#ef4444",
  orange: "#f97316",
...
const getDoorLabelStyles = (doorType: DoorType, hovered: boolean): CSSProperties => {
  const baseColor = doorBaseColors[doorType];
  const backgroundColor = adjustColor(baseColor, hovered ? 0.12 : 0.04);
  const color = getReadableTextColor(backgroundColor);
  const borderColor = adjustColor(baseColor, -0.35);
  const boxShadow = hovered ? `0 10px 22px ${hexToRgba(baseColor, 0.32)}` : undefined;
      ...
      <main className="relative flex-1">
        ...
        <div className="pointer-events-none absolute inset-0">
          {lobbyAreas.map((area) => {
            ...
            const labelStyle =
              area.isDoor && doorType ? getDoorLabelStyles(doorType, hovered) : undefined;
