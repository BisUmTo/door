...
const MedalsRoute = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0f1f1b] text-white">
      <div className="mx-auto flex min-h-screen w-full max-w-[1440px] flex-col px-6 py-10">
        <header className="flex items-center justify-between border-b border-white/10 pb-4">
          <div>
            ...
            <p className="mt-1 text-sm text-white/60">
              Colleziona tutte le medagliette aprendo le porte corrispondenti.
            </p>
          </div>
          <div className="flex items-center gap-3 text-sm">
            ...
            <Link
              to="/house"
              className="rounded-full border border-white/30 px-4 py-2 text-xs uppercase tracking-[0.35em] text-white/80 transition hover:border-accent hover:text-accent"
            >
              Torna alla casa
            </Link>
          </div>
        ...
        <section className="mt-10 grid flex-1 gap-6 rounded-3xl border border-white/10 bg-[#17302a]/80 p-6 backdrop-blur">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {MEDAL_DEFINITIONS.map((definition) => {
              ...
              return (
                <div
                  key={definition.type}
                  className="group relative flex flex-col items-center gap-3 rounded-3xl border border-white/5 bg-black/20 p-6 text-center transition hover:border-accent/50 hover:bg-black/40"
                >
                  <div className="relative flex h-28 w-28 items-center justify-center rounded-full border-4 border-white/10 bg-[#102720] shadow-inner transition group-hover:shadow-lg">
                    <div
                      className="h-24 w-24 rounded-full transition-all"
                      style={{
                        background: unlocked
                          ? `radial-gradient(circle at 30% 30%, #ffffff, ${definition.color})`
                          : `radial-gradient(circle at 30% 30%, #2f463f, #0f1f1b)`
                      }}
                    />
                    {!unlocked ? (
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-xs uppercase text-emerald-300/70">
